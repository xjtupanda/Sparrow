...
def load_video_from_file(video_path, num_frames=24):
    ...
    while cv2_vr.isOpened():
        ...
        count += 1
    cv2_vr.release()
    #video_data = video_data[::-1]    # exp: reverse the video
    return video_data
...
class CustomDataset(Dataset):
    def __init__(self, questions, video_folder, tokenizer, model_config, num_frames=24):
...
def create_data_loader(questions, video_folder, tokenizer, model_config, num_frames=24, batch_size=1, num_workers=4):
    ...
    dataset = CustomDataset(questions, video_folder, tokenizer, model_config, num_frames)
