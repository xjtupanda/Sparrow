...
path = "HuggingFaceM4/Idefics3-8B-Llama3"
tokenizer = AutoProcessor.from_pretrained(path, add_bos_token=True, add_eos_token=False, trust_remote_code=True).tokenizer

# LongAlpaca-pics  LongQLoRA-pics
# longalpaca_chat_fake_vid.json  longqlora_chat_fake_vid.json
orig_data = read_json("longalpaca_chat_fake_vid.json")
video_base_dir = "/data/pandayin/data/LongAlpaca-pics" 
new_data = []
...
for sample in tqdm(orig_data):
    ...
    if len(sample['conversations'][0]['value'].split(' ')) >= 500:  # 30 for LongQLoRA, 500 for LongAlpaca
        continue
    token_length += 169 * frame_count
...
write_json("idefics-longalpaca.json", new_data)
